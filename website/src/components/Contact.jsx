...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    ...
    // Build a mailto link — no backend required.
    // When the shared Supabase DB is wired in later, replace this block.
    const subject = encodeURIComponent(`Demo Request — ${form.restaurant}`)
    const body = encodeURIComponent(
      `Name: ${form.name}\nEmail: ${form.email}\nRestaurant: ${form.restaurant}\nSize: ${form.size}\n\n${form.message}`
    )
    window.location.href = `mailto:hello@paladeium.com?subject=${subject}&body=${body}`

    // Optimistically show success after mailto is triggered
    setSubmitted(true)
  }
            ...
            {submitted ? (
              <div className="text-center py-12 reveal-scale">
                ...
                <p className="text-sm" style={{ color: 'rgba(240,237,232,0.6)', fontFamily: 'Inter' }}>
                  Your email client should have opened. We'll reach out within 2 hours to schedule your demo.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  type="submit"
                  className="btn-gold w-full py-4 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 reveal-up delay-900"
